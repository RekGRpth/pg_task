...
На логическую структуру планировщика асинхронных заданий [pg_task](https://github.com/RekGRpth/pg_task) во многом повлиял планировщик задач из замечательного и очень необычного интересного питоновского веб-фреймворка [web2py](https://github.com/web2py/web2py).
В том планировщике задачи и и информация об их запусках хранятся в двух разных таблицах, а также повторяющаяся задача имеет одну запись в таблице задач и много записей в таблице запусков.
...
Что мне очень не нравится в планировщике web2py, так это то, что там должны быть постоянно запущены рабочие, которые выполняют задачи.
...
| --- | --- | --- | --- | --- |
| pg_task.delete | bool | true | конфигурация, база, пользователь | удалять задачу если нет результата и нет ошибки? |
| pg_task.drift | bool | false | конфигурация, база, пользователь | вычислять следующее время относительно времени завершения вместо времени планирования? |
| pg_task.header | bool | true | конфигурация, база, пользователь | показывать заголовки в результатах? |
| pg_task.string | bool | true | конфигурация, база, пользователь | квотировать только строки в результатах? |
...
| pg_task.count | int | 0 | конфигурация, база, пользователь | неотрицательное количество задач, выполняемое текущим фоновым рабочим процессом перед выходом |
...
| pg_task.max | int | 0 | конфигурация, база, пользователь | максимальное число одновременно выполняемых в группе задач, отрицательное значение означает паузу в миллисекундах между выполнениями задач в группе |
...
| pg_task.active | interval | 1 hour | конфигурация, база, пользователь | положительный период после запланированного времени, в течении которого задача актуальна для выполнения |
| pg_task.data | text | postgres | конфигурация | название базы данных для таблицы с заданиями |
| pg_task.delimiter | char | \t | конфигурация, база, пользователь | разделитель между колонками в результатах |
| pg_task.escape | char | | конфигурация, база, пользователь | символ экранирования в результатах |
| pg_task.group | text | group | конфигурация, база, пользователь | имя для группировки задач |
...
| pg_task.live | interval | 0 sec | конфигурация, база, пользователь | неотрицательный период жизни текущего фонового рабочего процесса для выполнения нескольких задач перед выходом |
| pg_task.null | text | \N | конфигурация, база, пользователь | заполнитель для NULL в результатах (и логах) |
| pg_task.quote | char | | конфигурация, база, пользователь | символ кавычек в результатах |
| pg_task.repeat | interval | 0 sec | конфигурация, база, пользователь | неотрицательный период автоматического повторения задачи |
...
| pg_task.timeout | interval | 0 sec | конфигурация, база, пользователь | неотрицательный период разрешённого времени выполнения задачи |
