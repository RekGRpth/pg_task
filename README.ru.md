...
Как уже было сказано выше, планировщик может выполнять любые SQL-запросы.
Здесь под SQL-запросами я понимаю не только `SELECT`, `UPDATE`, `DELETE`, `VACUUM`, `CREATE DATABASE` и т.д., но и более широкие возможности, реализуемые через различные расширения.
