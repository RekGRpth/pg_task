...
void conf_work(const char *user, const char *data, const char *schema, const char *table, const int reset, const int timeout, const int count, const int live) {
    ...
    snprintf(worker.bgw_function_name, sizeof(worker.bgw_function_name) - 1, "work_worker");
    snprintf(worker.bgw_library_name, sizeof(worker.bgw_library_name) - 1, "pg_task");
    snprintf(worker.bgw_type, sizeof(worker.bgw_type) - 1, "pg_task %s%s%s %i %i", schema ? schema : "", schema ? " " : "", table, reset, timeout);
    snprintf(worker.bgw_name, sizeof(worker.bgw_name) - 1, "%s %s %s", user, data, worker.bgw_type);
