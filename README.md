...
- **The `remote` column's password requirement only protects non-superuser `pg_task.user`.** When a task sets `remote`, `pg_task` refuses to connect without a password unless the *worker* process is a superuser (`work.c`'s `work_remote`/`work_connect`). That check looks at the privilege of the fixed `pg_task.user` role running the worker, not at the privilege of whoever inserted the task — so if `pg_task.user` is a superuser (the common case, see above), this protection is silently disabled for every task, and any task author can point `remote` at a passwordless/trust-authenticated server and pivot the worker's connection there. Don't rely on this check for isolation between task authors; it only helps when `pg_task.user` itself is unprivileged.
