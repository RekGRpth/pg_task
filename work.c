...
static void work_table(void) {
    ...
    appendStringInfo(&src, SQL(
        CREATE TABLE %1$s (
            ...
            active interval NOT NULL DEFAULT current_setting('pg_task.default_active', false)::interval,
            live interval NOT NULL DEFAULT current_setting('pg_task.default_live', false)::interval,
            repeat interval NOT NULL DEFAULT current_setting('pg_task.default_repeat', false)::interval,
            timeout interval NOT NULL DEFAULT current_setting('pg_task.default_timeout', false)::interval,
    ...
    if (!src.data) {
        ...
        appendStringInfo(&src, SQL(
            WITH s AS ( WITH l AS (
                ...
                WHERE t.state = 'PLAN'::%2$s AND t.plan + concat_ws(' ', (CASE WHEN t.max < 0 THEN -t.max ELSE 0 END)::text, 'msec')::interval <= CURRENT_TIMESTAMP
            ...
            ) UPDATE %1$s AS t SET state = 'TAKE'::%2$s FROM s WHERE t.id = s.id AND s.count > 0 RETURNING t.id, t.hash, t.group, t.remote, t.max
        ), work->schema_table, work->schema_type);
    }
    SPI_connect_my(src.data);
