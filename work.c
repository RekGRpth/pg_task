...
static void work_task(const Work *work, const int64 id, char *group, const int max) {
    ...
    snprintf(worker.bgw_function_name, sizeof(worker.bgw_function_name) - 1, "task_worker");
    snprintf(worker.bgw_library_name, sizeof(worker.bgw_library_name) - 1, "pg_task");
    snprintf(worker.bgw_type, sizeof(worker.bgw_type) - 1, "pg_task %s%s%s %s", work->schema ? work->schema : "", work->schema ? " " : "", work->table, group);
    snprintf(worker.bgw_name, sizeof(worker.bgw_name) - 1, "%s %s %s", work->user, work->data, worker.bgw_type);
