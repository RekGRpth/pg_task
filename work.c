    ...
    if (!src.data) {
        ...
        appendStringInfo(&src, SQL(
           SELECT COALESCE(LEAST(EXTRACT(epoch FROM ((
                ...
                LEFT JOIN "pg_catalog"."pg_locks" AS l ON "locktype" = 'userlock' AND "mode" = 'AccessExclusiveLock' AND "granted" AND "objsubid" = 4 AND "database" = %2$i AND "classid" = "id" OPERATOR(pg_catalog.>>) 32 AND "objid" = "id" OPERATOR(pg_catalog.<<) 32 OPERATOR(pg_catalog.>>) 32
