    ...
    if (!src.data) {
        ...
        appendStringInfo(&src, SQL(
            WITH l AS (
                ...
                WHERE "plan" + pg_catalog.concat_ws(' ', (-CASE WHEN "max" OPERATOR(pg_catalog.>=) 0 THEN 0 ELSE "max" END)::pg_catalog.text, 'msec')::pg_catalog.interval <= CURRENT_TIMESTAMP AND "state" OPERATOR(pg_catalog.=) 'PLAN' AND CASE WHEN "max" OPERATOR(pg_catalog.>=) 0 THEN "max" ELSE 0 END - COALESCE("classid", 0) OPERATOR(pg_catalog.>=) 0
