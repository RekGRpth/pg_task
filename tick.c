...
static void register_task_worker(const Datum id, const char *queue, const uint64 max) {
    ...
    if (snprintf(worker.bgw_library_name, sizeof("pg_task"), "pg_task") != sizeof("pg_task") - 1) ereport(ERROR, (errmsg("%s(%s:%d): snprintf != %lu", __func__, __FILE__, __LINE__, sizeof("pg_task") - 1)));
    if (snprintf(worker.bgw_function_name, sizeof("task_worker"), "task_worker") != sizeof("task_worker") - 1) ereport(ERROR, (errmsg("%s(%s:%d): snprintf != %lu", __func__, __FILE__, __LINE__, sizeof("task_worker") - 1)));
    len = (sizeof("pg_task task %s") - 1) + (strlen(queue) - 1) - 1;
    if (len + 1 > BGW_MAXLEN) ereport(ERROR, (errmsg("%s(%s:%d): %lu > BGW_MAXLEN", __func__, __FILE__, __LINE__, len + 1)));
    if (snprintf(worker.bgw_type, len + 1, "pg_task task %s", queue) != len) ereport(ERROR, (errmsg("%s(%s:%d): snprintf != %lu", __func__, __FILE__, __LINE__, len)));
    len = (sizeof("%s %s pg_task task %s") - 1) + (strlen(username) - 1) + (strlen(database) - 1) + (strlen(queue) - 1) - 1 - 1 - 1;
    if (len + 1 > BGW_MAXLEN) ereport(ERROR, (errmsg("%s(%s:%d): %lu > BGW_MAXLEN", __func__, __FILE__, __LINE__, len + 1)));
    if (snprintf(worker.bgw_name, len + 1, "%s %s pg_task task %s", username, database, queue) != len) ereport(ERROR, (errmsg("%s(%s:%d): snprintf != %lu", __func__, __FILE__, __LINE__, len)));
    ...
    if (database_len + 1 + username_len + 1 + schema_len + 1 + table_len + 1 + queue_len + 1 + sizeof(uint64)> BGW_EXTRALEN) ereport(ERROR, (errmsg("%s(%s:%d): %lu > BGW_EXTRALEN", __func__, __FILE__, __LINE__, database_len + 1 + username_len + 1 + schema_len + 1 + table_len + 1 + queue_len + 1 + sizeof(uint64))));
    ...
    if (schema && snprintf(worker.bgw_extra + database_len + 1 + username_len + 1, schema_len + 1, "%s", schema) != schema_len) ereport(ERROR, (errmsg("%s(%s:%d): snprintf != %lu", __func__, __FILE__, __LINE__, schema_len)));
