...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..90 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 90 x pg_sleep(1) waiting for root task in group ''stop_cancels_repeat'' to reach WORK state'; END IF;
...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..15 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 15 x pg_sleep(1) waiting for root task in group ''stop_cancels_repeat'' to reach STOP state'; END IF;
END;$body$ LANGUAGE plpgsql;
