...
DO $body$ BEGIN
    FOR i IN 1..90 LOOP
        IF (SELECT state FROM task WHERE "group" = 'stop_cancels_repeat' AND parent IS NULL) = 'WORK' THEN EXIT; END IF;
        PERFORM pg_sleep(1);
    END LOOP;
...
DO $body$ BEGIN
    FOR i IN 1..15 LOOP
        IF (SELECT state FROM task WHERE "group" = 'stop_cancels_repeat' AND parent IS NULL) = 'STOP' THEN EXIT; END IF;
        PERFORM pg_sleep(1);
    END LOOP;
END;$body$ LANGUAGE plpgsql;
