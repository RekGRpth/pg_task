...
DO $body$ DECLARE ok boolean := false; BEGIN
    ...
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 300 x pg_sleep(0.1) waiting for column ''delimiter'' on task_column_drift_test_schema.task_column_drift_test to be healed back to a single non-dropped column'; END IF;
