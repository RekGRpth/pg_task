...
INSERT INTO pg_task_test_state VALUES ('base_json', :'base_json'), ('base_json_added', :'base_json_added'), ('start_ct', quote_literal(CURRENT_TIMESTAMP));
ALTER SYSTEM SET pg_task.json = :'json_val';
SELECT pg_reload_conf();
DO $body$ BEGIN
    FOR i IN 1..120 LOOP
        IF to_regclass('public.task') IS NOT NULL AND EXISTS (SELECT 1 FROM pg_catalog.pg_stat_activity WHERE application_name LIKE 'pg_work public task %' AND datname = current_database() AND state = 'idle') THEN EXIT; END IF;
        PERFORM pg_sleep(1);
    END LOOP;
