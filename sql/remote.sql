...
DO $body$ BEGIN
    ...
    END LOOP;
END;$body$ LANGUAGE plpgsql;
SELECT "group", min(start) - :ct13::timestamptz < interval '2500 ms' AS first_run_immediate, bool_and(gap IS NULL OR gap >= interval '2 sec') AS pause_ok FROM (
...
DO $body$ BEGIN
    ...
    END LOOP;
END;$body$ LANGUAGE plpgsql;
SELECT "group", min(start) - :ct14::timestamptz < interval '2500 ms' AS first_run_immediate, bool_and(gap IS NULL OR gap >= interval '2 sec') AS pause_ok FROM (
