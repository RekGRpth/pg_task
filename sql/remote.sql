...
BEGIN;
WITH s AS (SELECT generate_series(1, 10) AS s) INSERT INTO task ("group", input, max, live, remote) SELECT '9', 'SELECT pg_sleep(0.1) AS a', 2, '1 min', 'application_name=test' FROM s;
INSERT INTO task ("group", input, max, live, remote) VALUES ('9', 'SELECT pg_sleep(0.1) AS a', 3, '1 min', 'application_name=test');
COMMIT;
DO $body$ BEGIN
    WHILE true LOOP
        PERFORM pg_sleep(0.1);
