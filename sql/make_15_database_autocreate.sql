...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..30 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 30 x pg_sleep(1) waiting for database ''task_make_data_test'' to be auto-created'; END IF;
...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..30 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 30 x pg_sleep(1) waiting for backend(s) connected as role ''task_make_data_test'' to disconnect'; END IF;
END;$body$ LANGUAGE plpgsql;
