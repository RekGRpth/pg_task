...
\gset
INSERT INTO task ("group", input, repeat, drift) VALUES ('repeat_drift', 'SELECT pg_sleep(1) AS a', '3 sec', true);
DO $body$ BEGIN
    FOR i IN 1..90 LOOP
        IF (SELECT count(*) FILTER (WHERE state = 'DONE') >= 3 FROM task WHERE "group" = 'repeat_drift') THEN EXIT; END IF;
        PERFORM pg_sleep(1);
    END LOOP;
...
DO $body$ BEGIN
    FOR i IN 1..120 LOOP
        IF (SELECT count(*) FROM task WHERE "group" = 'repeat_drift' AND state NOT IN ('DONE', 'GONE', 'FAIL')) = 0 THEN EXIT; END IF;
        PERFORM pg_sleep(1);
    END LOOP;
END;$body$ LANGUAGE plpgsql;
