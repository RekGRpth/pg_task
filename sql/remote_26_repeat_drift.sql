...
\gset
INSERT INTO task ("group", input, repeat, drift, remote) VALUES ('repeat_drift', 'SELECT pg_sleep(0.2) AS a', '1 sec', true, 'dbname=' || :'DBNAME');
...
SELECT count(*) >= 3 AS repeated_enough,
    ...
    bool_and(gap IS NULL OR gap BETWEEN interval '900 ms' AND interval '1500 ms') AS drift_from_stop_ok
FROM g;
