...
WITH s AS (SELECT generate_series(1, 3) AS s) INSERT INTO task ("group", input, max, drift) SELECT 'pause_drift', 'SELECT clock_timestamp() AS a', -1000, true FROM s;
...
SELECT "group", min(start) - :ct13::timestamptz < interval '2500 ms' AS first_run_immediate, bool_and(gap IS NULL OR gap BETWEEN interval '700 ms' AND interval '3 sec') AS pause_ok FROM (
