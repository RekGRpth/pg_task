...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..30 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 30 x pg_sleep(1) waiting for remote backend running SELECT pg_sleep(30) to appear in pg_stat_activity'; END IF;
...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..30 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 30 x pg_sleep(1) waiting for task group ''connection_kill_recovery'' (input ''SELECT pg_sleep(30)'') to leave PLAN/TAKE/WORK after its connection is killed'; END IF;
END;$body$ LANGUAGE plpgsql;
...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..30 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 30 x pg_sleep(1) waiting for task group ''connection_kill_recovery'' (input ''SELECT 1 AS a'') to finish (leave PLAN/TAKE/WORK)'; END IF;
END;$body$ LANGUAGE plpgsql;
