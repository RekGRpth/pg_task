...
SELECT "group", input, output, state, count(id) FROM task WHERE "group" = '7' AND input like 'SELECT pg_sleep(0.1) AS a' AND start >= :ct::timestamp GROUP BY "group", input, output, state, pid;
