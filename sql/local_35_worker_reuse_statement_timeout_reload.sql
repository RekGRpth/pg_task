...
DO $body$ DECLARE ok boolean := false; BEGIN
    ...
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 150 x pg_sleep(0.1) waiting for at least 1 DONE task(s) in group ''worker_reuse_timeout_reload'''; END IF;
...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..150 LOOP
        ...
        PERFORM pg_sleep(0.1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 150 x pg_sleep(0.1) waiting for task group ''worker_reuse_timeout_reload'' to finish (leave PLAN/TAKE/WORK)'; END IF;
END;$body$ LANGUAGE plpgsql;
