...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..30 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 30 x pg_sleep(1) waiting for at least one task in groups ''limit_sys_a'', ''limit_sys_b'', ''limit_sys_c'' to leave PLAN state'; END IF;
...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..30 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 30 x pg_sleep(1) waiting for task groups ''limit_sys_a'', ''limit_sys_b'', ''limit_sys_c'' to finish (leave PLAN/TAKE/WORK)'; END IF;
END;$body$ LANGUAGE plpgsql;
DELETE FROM task WHERE "group" IN ('limit_sys_a', 'limit_sys_b', 'limit_sys_c');
