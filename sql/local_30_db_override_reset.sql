...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..15 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 15 x pg_sleep(1) waiting for task in group ''db_override_reset'' to reach WORK state'; END IF;
...
DO $body$ DECLARE ok boolean := false; BEGIN
    FOR i IN 1..30 LOOP
        ...
        PERFORM pg_sleep(1);
    END LOOP;
    IF NOT ok THEN RAISE EXCEPTION 'timed out after 30 x pg_sleep(1) waiting for task in group ''db_override_reset'' to reach DONE state'; END IF;
END;$body$ LANGUAGE plpgsql;
