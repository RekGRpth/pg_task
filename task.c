    ...
    if (!src.data) {
        ...
        appendStringInfo(&src, SQL(
            WITH s AS (
                ...
                WHERE "plan" + pg_catalog.concat_ws(' ', (-"max")::pg_catalog.text, 'msec')::pg_catalog.interval OPERATOR(pg_catalog.<=) CURRENT_TIMESTAMP AND "state" OPERATOR(pg_catalog.=) 'PLAN' AND "hash" OPERATOR(pg_catalog.=) $1 AND "max" < 0 FOR UPDATE OF t
