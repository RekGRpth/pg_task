...
static void task_work(void) {
    ...
    if (!command) {
        ...
        appendStringInfo(&buf,
            ...
            "set_config('pg_task.oid', concat_ws('.', NULLIF(current_setting('pg_task.schema', true), ''), current_setting('pg_task.table', false))::regclass::oid::text, false)", schema_quote, point, table_quote);
        command = buf.data;
    }
    #undef ID
    #undef SID
...
static void task_repeat(void) {
    ...
    if (!command) {
        ...
        appendStringInfo(&buf,
            ...
            "FROM %1$s%2$s%3$s WHERE id = $" SID " AND state IN ('DONE'::state, 'FAIL'::state) LIMIT 1", schema_quote, point, table_quote);
        command = buf.data;
    }
    #undef ID
    #undef SID
...
static void task_live(void) {
    ...
    if (!command) {
        ...
        appendStringInfo(&buf,
            ...
            ") UPDATE %1$s%2$s%3$s AS u SET state = 'TAKE'::state FROM s WHERE u.id = s.id RETURNING u.id, set_config('pg_task.id', u.id::text, false)", schema_quote, point, table_quote);
        command = buf.data;
    }
    #undef QUEUE
    #undef SQUEUE
...
static void task_done(void) {
    ...
    if (!command) {
        ...
        appendStringInfo(&buf,
            ...
            "pg_advisory_unlock(current_setting('pg_task.oid', true)::int4, $" SID "::int4)", schema_quote, point, table_quote);
        command = buf.data;
    }
    #undef ID
    #undef SID
