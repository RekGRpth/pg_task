...
static bool task_live(const Task *t) {
    ...
    if (!src.data) {
        ...
        appendStringInfo(&src, SQL(
            ...
            END ORDER BY "max" DESC, "id" LIMIT 1 FOR UPDATE OF t %2$s) UPDATE %1$s AS t SET "state" = 'TAKE' FROM s WHERE t.id OPERATOR(pg_catalog.=) s.id RETURNING t.id
        ), work.schema_table,
#if PG_VERSION_NUM >= 90500
        "SKIP LOCKED"
#else
...
static void task_columns(const Task *t) {
    ...
    static const char *src = SQL(
        SELECT pg_catalog.string_agg(quote_ident(column_name), ', ') AS columns FROM information_schema.columns WHERE table_schema OPERATOR(pg_catalog.=) $1 AND table_name OPERATOR(pg_catalog.=) $2 AND column_name NOT IN ('id', 'plan', 'parent', 'start', 'stop', 'hash', 'pid', 'state', 'error', 'output')
    );
    ...
    if (!src.data) {
        ...
        appendStringInfo(&src, SQL(
            WITH s AS (
                ...
                WHERE "plan" + pg_catalog.concat_ws(' ', (-"max")::pg_catalog.text, 'msec')::pg_catalog.interval <= CURRENT_TIMESTAMP AND "state" = 'PLAN' AND "hash" = $1 AND "max" < 0 FOR UPDATE OF t
            ) UPDATE %1$s AS t SET "plan" = CASE WHEN "drift" THEN CURRENT_TIMESTAMP ELSE (WITH RECURSIVE r AS (SELECT "plan" AS p UNION SELECT p + pg_catalog.concat_ws(' ', (-"max")::pg_catalog.text, 'msec')::pg_catalog.interval FROM r WHERE p <= CURRENT_TIMESTAMP) SELECT * FROM r ORDER BY 1 DESC LIMIT 1) END FROM s
